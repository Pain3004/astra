    ...
    public function restoreExternalCarrier(Request $request)
    {
        ...
        foreach($custID as $company_id)
        {
            ...
            foreach($cardIds as $r)
            {
                ...
                ',' , ' " " ', '[', ']' ), ' ', $r);
                $r = preg_replace('/\s+/', ' ', $r);
                $Shipper=Carrier::raw()->updateOne(['companyID' =>$companyID,'_id' => $masterId,'carrier._id' => (int)$r], 
                ['$set' => ['carrier.$.deleteStatus' => 'NO','carrier.$.deleteUser' => Auth::user()->userName,'carrier.$.deleteTime' => time()]]
                ); 
            }
